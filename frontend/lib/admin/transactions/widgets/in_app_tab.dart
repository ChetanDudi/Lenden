...
class InAppCat {
  ...
  final IconData icon;
  final Color color;
  const InAppCat(this.key, this.label, this.icon, this.color);
}

const List<InAppCat> kInAppCats = [
  InAppCat('all',          'All',          Icons.swap_horiz_rounded,            AppColors.cyan),
  InAppCat('p2p',          'P2P',          Icons.people_alt_rounded,            Color(0xFF1E88E5)),
  InAppCat('quick',        'Quick Tx',     Icons.flash_on_rounded,              Color(0xFFFB8C00)),
  InAppCat('secure',       'Secure Tx',    Icons.shield_rounded,                Color(0xFF43A047)),
  InAppCat('group',        'Group',        Icons.group_work_rounded,            Color(0xFF8E24AA)),
  InAppCat('qr',           'QR/Scan',      Icons.qr_code_2_rounded,             Color(0xFF00897B)),
  InAppCat('subscription', 'Plans',        Icons.workspace_premium_rounded,     Color(0xFFE53935)),
  InAppCat('coins',        'Coins',        Icons.toll_rounded,                  Color(0xFFFFB300)),
  InAppCat('rewards',      'Rewards',      Icons.stars_rounded,                 Color(0xFF6A1B9A)),
  InAppCat('gift_cards',   'Gift Cards',   Icons.card_giftcard_rounded,         Color(0xFFE91E63)),
  InAppCat('offer_coins',  'Offer Coins',  Icons.local_offer_rounded,           Color(0xFF009688)),
  InAppCat('referrals',    'Referrals',    Icons.group_add_rounded,             Color(0xFF9C27B0)),
  InAppCat('refunds',      'Refunds',      Icons.undo_rounded,                  Color(0xFF00BCD4)),
];
...
class InAppTab extends StatefulWidget {
  final InAppCat cat;
  final Map<String, int> catCounts;
  const InAppTab({required this.cat, required this.catCounts});

  @override
  State<InAppTab> createState() => _InAppTabState();
}

class _InAppTabState extends State<InAppTab> with AutomaticKeepAliveClientMixin {
  @override
  ...
  Widget _quickTxCard(BuildContext context, Map<String, dynamic> qt) {
    ...
    final cat = kInAppCats.firstWhere((c) => c.key == 'quick');
  ...
  Widget _coinLedgerCard(BuildContext context, Map<String, dynamic> entry) {
    ...
    final rewardCat = kInAppCats.firstWhere((c) => c.key == 'rewards');
  ...
  InAppCat _detectCat(Map<String, dynamic> tx) {
    ...
    if (stMap.containsKey(st)) {
      return kInAppCats.firstWhere((c) => c.key == stMap[st]!);
    }
    ...
    if (note.contains('quick transaction')) return kInAppCats.firstWhere((c) => c.key == 'quick');
    if (note.contains('secure transaction')) return kInAppCats.firstWhere((c) => c.key == 'secure');
    if (note.contains('group expense')) return kInAppCats.firstWhere((c) => c.key == 'group');
    if (note.contains('qr payment') || hasScan) return kInAppCats.firstWhere((c) => c.key == 'qr');
    if (note.contains('subscription')) return kInAppCats.firstWhere((c) => c.key == 'subscription');
    if (note.contains('lenden coin') ||
        (note.contains('bought') && note.contains('coin'))) {
      return kInAppCats.firstWhere((c) => c.key == 'coins');
    }
    if (note.contains('refund') || note.contains('reversed')) {
      return kInAppCats.firstWhere((c) => c.key == 'refunds');
    }
    if (note.contains('wallet transfer')) return kInAppCats.firstWhere((c) => c.key == 'p2p');
    if (widget.cat.key != 'all') return widget.cat;
    return kInAppCats.first;
  }
