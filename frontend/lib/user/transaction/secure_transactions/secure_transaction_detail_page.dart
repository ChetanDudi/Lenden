...
class _SecureTransactionDetailPageState
  ...
  Future<void> _shareTransactionAsPdf() async {
    ...
    final txId     = (t['transactionId'] ?? shortId).toString();
    final dateStr  = (t['date'] ?? '').toString();
    final timeStr  = (t['time'] ?? '').toString();

    final session  = Provider.of<SessionProvider>(context, listen: false).user;
    final myEmail  = (session?['email'] ?? '').toString();
    ...
    final interestStr = (t['interestType'] != null && t['interestRate'] != null)
        ? '${t['interestType']} @ ${t['interestRate']}%' : '';
    final remaining = _formatDisplayAmount(
        double.tryParse(_calculateRemainingAmount(t)) ?? 0, currency);
      ...
      build: (ctx) => [
        pw.Container(
          ...
          child: pw.Column(crossAxisAlignment: pw.CrossAxisAlignment.start, children: [
            ...
            pw.Row(mainAxisAlignment: pw.MainAxisAlignment.spaceBetween, children: [
              pw.Text('Ref: #$txId', style: pw.TextStyle(color: white70, fontSize: 10)),
        ...
        pw.Container(
          decoration: pw.BoxDecoration(
            color: isLend ? const PdfColor.fromInt(0xFFE8F5E9) : const PdfColor.fromInt(0xFFFFEBEE),
            borderRadius: const pw.BorderRadius.all(pw.Radius.circular(10)),
            border: pw.Border.all(color: isLend ? green : red, width: 1.5),
          ),
          padding: const pw.EdgeInsets.all(16),
          child: pw.Row(mainAxisAlignment: pw.MainAxisAlignment.spaceBetween, children: [
            pw.Column(crossAxisAlignment: pw.CrossAxisAlignment.start, children: [
              pw.Text(isLend ? 'You Lent' : 'You Borrowed',
                  style: pw.TextStyle(fontSize: 11, color: isLend ? green : red)),
              pw.SizedBox(height: 4),
              pw.Text('$sym ${amount.toStringAsFixed(2)}',
                  style: pw.TextStyle(fontSize: 22, fontWeight: pw.FontWeight.bold, color: isLend ? green : red)),
            ]),
            pw.Column(crossAxisAlignment: pw.CrossAxisAlignment.end, children: [
              ...
              if (remaining.isNotEmpty) ...[
                ...
                pw.Text('Remaining: $remaining', style: pw.TextStyle(fontSize: 9, color: const PdfColor(0.4, 0.4, 0.4))),
              ],
            ]),
          ]),
        ),
        ...
        pw.Container(
          ...
          child: pw.Column(children: [
            ...
            if (interestStr.isNotEmpty) ...[
              pw.Divider(color: const PdfColor(0.85, 0.85, 0.85)),
              rowItem('Interest', interestStr),
            ],
            pw.Divider(color: const PdfColor(0.85, 0.85, 0.85)),
    ...
    await shareBytesFile(bytes: bytes, filename: 'lenden_secure_$shortId.pdf',
        mimeType: 'application/pdf', subject: 'Secure Transaction Receipt – #$txId');
  }
