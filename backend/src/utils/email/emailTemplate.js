const shell = (content, isAdmin = false) => `
  ...
  <tr><td align="center">
    <table width="580" cellpadding="0" cellspacing="0" border="0" style="background:#ffffff;border:1px solid #cccccc;font-family:Arial,Helvetica,sans-serif;">
      <tr><td style="background:#003d75;padding:20px 32px;">
        <span style="font-size:20px;font-weight:bold;color:#ffffff;letter-spacing:1px;">LenDen</span>${isAdmin ? '<span style="font-size:12px;color:#aaccee;margin-left:10px;">Admin Portal</span>' : ''}
      </td></tr>
...
const otpBlock = (otp, expiryMinutes = 2) => `
<div style="background:#f7f7f7;border:1px solid #dddddd;border-left:4px solid #003d75;margin:24px 0;padding:18px 24px;text-align:center;">
  ...
  <p style="margin:8px 0 0;font-size:12px;color:#999999;">This code expires in ${expiryMinutes} minutes.</p>
</div>`;

const infoRow = (label, value, shaded) =>
...
const infoTable = (rows) =>
  `<table width="100%" cellpadding="0" cellspacing="0" border="0" style="border:1px solid #e0e0e0;margin-bottom:16px;">
    ${rows.map(([label, value], i) => infoRow(label, value, i % 2 === 0)).join('')}
  </table>`;

const alertColors = {
  error:   { bg: '#fce8e8', border: '#cc0000' },
  warning: { bg: '#fff3cd', border: '#cc8800' },
  success: { bg: '#e8f5e9', border: '#388e3c' },
  info:    { bg: '#e3f2fd', border: '#1565c0' },
};

const alertBox = (message, type) => {
  const c = alertColors[type] || alertColors.info;
  return `<p style="margin:16px 0 0;background:${c.bg};border-left:4px solid ${c.border};padding:12px 16px;color:#555555;font-size:13px;">${message}</p>`;
};

exports.shell = shell;
exports.otpBlock = otpBlock;
