...
exports.dismissNotification = async (req, res) => {
  try {
    const userId = req.user._id;
    const { id } = req.params;
    const notification = await Notification.findOne({ _id: id, recipients: userId });
    if (!notification) {
      return res.status(404).json({ message: 'Notification not found in your inbox' });
    }
    await Notification.updateOne({ _id: id }, { $pull: { recipients: userId } });
    // Clean up orphan documents that no one receives anymore
    await Notification.deleteMany({ _id: id, recipients: { $size: 0 } });
